...
通过后再启用自动化，不必开放全部工具权限。另用 `get_news(category="social", hours=72, limit=3)` 验证社媒摘要：应保留 `provenance`，链接和日期应标为摘要的链接和发布时间，不是 X/Reddit 原帖信息。
...
步骤：
1. 先调用 get_collection_status，从 configured_sources 查看本次支持的来源与分类。
2. 若 stale=true、最近一轮全部失败或部分来源失败，在日报开头明确标出最后成功时间与缺失来源。
3. 先按 social，再按 official_ai、engineering、research、releases、media、community 共七个 category 调用 get_news，每次 hours=24、limit=20、per_source_limit=2。优先检查社媒摘要，避免它在候选预算用完前被跳过；其余分类保持跨来源覆盖，不让高频媒体挤掉论文和开源项目。确实需要更多时用 next_offset 翻页且保持其他参数不变，或按 source 单独取最多 10 条。全部读取不超过 100 条候选，达到上限就说明本次覆盖范围，避免无限调用和积分消耗。
4. 优先选出 AI 模型、AI 编程、开源工具、芯片和重要科技进展，共 5～10 条；不足 5 条就按实际数量输出。有合适内容时覆盖不同分类，不强行凑齐每类。
5. 相同事件尽量合并，但不要把不同事件强行合并。不把 HN 热门或讨论数量当成事实核实。
6. 每条给中文标题、最多两句中文摘要、来源、发布时间与内容链接。provenance.kind=aggregated_digest 时单列“社媒聚合摘要”，标明“经 AINews 转述”，published_at 只称“摘要发布时间”，url 只称“摘要链接”，不称原帖。摘录中的 X/Reddit 标签只说明本次可见片段；不要从 provenance.platforms 推断每期两平台都完整覆盖。不编造作者、原帖链接、发帖时间、投票数或 Discord/Telegram 讨论。社媒摘要可能含错误、传闻和过时观点，不能当成官方确认；同一事件有官方源时优先核对。依据只有标题或短摘录时明确说明，不编造性能数字、定价或发布日期。
