...
3. 分别按 official_ai、engineering、research、releases、media、community 六个 category 调用 get_news，每次 hours=24、limit=20、per_source_limit=2。这样先获取跨来源的候选，不让高频媒体挤掉论文和开源项目。确实需要更多时用 next_offset 翻页且保持其他参数不变，或按 source 单独取最多 10 条。全部读取不超过 100 条候选，达到上限就说明本次覆盖范围，避免无限调用和积分消耗。
